...
class EnergyLossTorch(nn.Module):
    def __init__(self, batch, cell_range=16):
        super(EnergyLossTorch, self).__init__()
        self.batch = batch
        self.cell_range = cell_range
        self.bins_num = 256 / cell_range
        self.register_buffer('bins_idx', torch.zeros(self.batch, self.bins_num))
    def forward(self, fea_out, fea_in):
        mat_index = (((fea_in +1) *127) / self.cell_range).int()
        mat_feature = fea_out - fea_in
        #print fea_out.device
        self.bins_idx = torch.zeros(self.batch, self.bins_num).to(fea_out.device)
        for bs in range(self.batch):
            fea_map = mat_feature[bs,0]
            for idx in range(self.bins_num):
                num = torch.sum(mat_index[bs,0]==idx)
                if num >0:
                    self.bins_idx[bs, idx] = torch.mean(fea_map[mat_index[bs,0]==idx])
        diff_abs = torch.abs(self.bins_idx[:,1:] - self.bins_idx[:,:-1])
